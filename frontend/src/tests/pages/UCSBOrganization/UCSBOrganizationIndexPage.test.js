...
describe("UCSBOrganizationIndexPage tests", () => {
    ...
    test("renders three orgs correctly for regular user", async () => {
        ...
        await waitFor(() => { expect(screen.getByTestId(`${testId}-cell-row-0-col-orgCode`)).toHaveTextContent(ucsbOrganizationFixtures.threeOrganizations[0].orgCode); });
        ...
        expect(screen.getByTestId(`${testId}-cell-row-2-col-orgCode`)).toHaveTextContent(ucsbOrganizationFixtures.threeOrganizations[2].orgCode);

        const createBotton = screen.queryByText("Create UCSB Organization");
        expect(createBotton).not.toBeInTheDocument();

        const orgCode = screen.getByText(ucsbOrganizationFixtures.threeOrganizations[0].orgCode);
        expect(orgCode).toBeInTheDocument();

        const orgTranslationShort = screen.getByText(ucsbOrganizationFixtures.threeOrganizations[0].orgTranslationShort);
